...
class _GraphIOSharedMixin:
    ...
    # ensure the alias is kept if someone clears and recreates sockets
    ...
    # optional: keep outputs identical in the saved representation
    def to_dict(
        self, short: bool = False, should_serialize: bool = False
    ) -> Dict[str, Any]:
        data = super().to_dict(short=short, should_serialize=should_serialize)
        if not short:
            # duplicate structure so readers that expect both keys still work
...
class GraphLevelNode(_GraphIOSharedMixin, SpecNode):
    ...
    catalog = "Builtins"
    is_dynamic: bool = True  # if you rely on this flag elsewhere
