...
class _SpecBackedMixin:
    ...
    # call this from your __init__ after super().__init__(...)
    def _init_with_spec(self, spec: NodeSpec, *, embed_spec: bool = True) -> None:
        ...
        if getattr(spec, "properties", None):
            self.properties._clear()
            for pname, pdef in spec.properties.items():
                if isinstance(pdef, dict):
                    kw = dict(pdef)
                    kw.setdefault("name", pname)
                    self.add_property(**kw)
                else:
...
# your existing SpecNode can now be simplified
