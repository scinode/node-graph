...
def _spec_shape_snapshot(spec: SocketSpec) -> Dict[str, Any]:
    """
    ...
    if spec.identifier == type_mapping["namespace"]:
        # fixed fields
        if spec.fields:
            d["sockets"] = {k: _spec_shape_snapshot(v) for k, v in spec.fields.items()}

        # dynamic behavior
        if spec.dynamic:
            d["dynamic"] = True
            if spec.item is not None:
                d["item"] = _spec_shape_snapshot(spec.item)
            else:
                # fallback to 'any' when item is missing
                d["item"] = {"identifier": type_mapping.get("any", "node_graph.any")}

        # carry defaults (optional, handy for UIs)
        if spec.defaults:
...
def runtime_meta_from_spec(
    ...
    *,
    ...
    # ---- UI/shape hints into extras ----
    extras: Dict[str, Any] = _spec_shape_snapshot(spec)

    # widget hint
    if spec.meta.widget is not None:
        extras["widget"] = spec.meta.widget

    # allow callers to override/augment
    if extra_overrides:
