# socket_spec.py — all-in-one, classmethod-based authoring + full inference
...
class SocketSpec:
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        if any(
            ...
            for k in ("help", "required", "widget", "call_role")
        ):
            d["meta"] = {
    ...
    def from_namespace(
        ...
        def _iter_ns_items(ns):
            ...
            if hasattr(ns, "sockets") and isinstance(getattr(ns, "sockets"), dict):
                return ns.sockets.items()
            try:
...
# ---------------- External fluent view (kept outside API class) ----------------
...
# ---------------- Classmethod-based authoring API ----------------
    ...
    def namespace(cls, _name: str | None = None, /, **fields) -> SocketSpec:
        ...
        for name, val in fields.items():
            ...
            child = (
                ...
                else SocketSpec(identifier=cls._map_identifier(base_T), meta=s_meta)
            )
    ...
    def build_outputs_from_signature(
        ...
        if explicit is not None:
            ...
            if cls._is_namespace(explicit):
                # if author supplied an *empty* non-dynamic namespace, give it a default 'result: Any'
...
def merge_spec(ns: SocketSpec, additions: SocketSpec) -> SocketSpec:
    """
...
# ---------------- Compatibility helpers (free funcs) ----------------
