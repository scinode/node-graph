...
# ------------
#
# In ``node-graph``, a critical feature is tracking task inputs and outputs to ensure data provenance and reproducibility.
# To achieve this, task functions must be annotated with input and output specifications.
# This tells the engine how to handle, serialize, and store data as individual data nodes.
#
...
#    enabling arbitrary nested structures (e.g., dictionaries). Each value will then be
#    recursively treated as a leaf node.
#
# Nested namespaces
# ~~~~~~~~~~~~~~~~~
#
...
# %%
# The summary confirms that the output is correctly structured with a top-level ``sum``
# and a nested ``nested`` dictionary, just as defined in the namespace.
#
...
#
# If you want a **dynamic** namespace, set ``model_config = {"extra": "allow"}`` and
# (optionally) ``"item_type"`` for the type of each dynamic value.
# If you want to treat a model as a **single leaf** (blob), set ``model_config = {"leaf": True}`` or
# use ``Leaf[YourModel]`` in the annotation.
...
# At the workflow level, a ``task.graph`` can define its own inputs and outputs, providing a clean
# interface to a complex chain of tasks.
# `node-graph` validates data against these graph-level specifications and automatically links
# graph inputs to the appropriate task inputs.
#
# In this final example, we will build a graph that reuses the input and output specifications
# from the tasks it contains.
...
#   Here we reuse ``generate_square_numbers.outputs`` and ``add_multiply.outputs`` to ensure the
#   graph's output signature is consistent with the tasks it contains.
#
...
# .. Important::
#
#     To apply a ``select``, you must place it in the metadata list of ``t.Annotated[<type>, ...]`` alongside
#     the specification you are modifying.
#
...
# and the nested inputs ``inputs.consume_complex1.data.pw`` and ``inputs.consume_complex2.data.pw`` are
# missing the ``structure`` socket, just as we specified.
#
