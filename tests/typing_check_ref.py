"""Typed usage of ``ref()`` on a TypedDict-annotated graph body.
...
hatch. ``codes.ref("pw")`` on the same ``Codes`` value is an attr-defined
error under ``--strict``, since ``TypedDict`` declares no ``ref`` method —
that is the gap this function closes.
"""
