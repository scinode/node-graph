...
def test_link() -> None:
    ...
    ng = NodeGraph(name="test_group_inputs_outputs")
    ng.group_inputs = {"x": 1.0, "y": 2.0}
    node1 = ng.add_node(NodePool.node_graph.test_add, "add1", x=ng.group_inputs.x)
    ng.add_node(
    ...
    ng.group_outputs.sum = ng.nodes.add2.outputs.result
    assert len(ng.meta_nodes) == 3
...
def test_from_dict() -> None:
    """Test the group inputs and outputs of a node graph."""
    ng = NodeGraph(name="test_group_inputs_outputs")
    ng.group_inputs = {"x": 1.0}
    ng.ctx = {"y": 2.0}
    node1 = ng.add_node(NodePool.node_graph.test_add, "add1", x=ng.group_inputs.x)
    ng.ctx.sum1 = node1.outputs.result
    ng.group_outputs.sum1 = ng.ctx.sum1
    ng.add_node(
        NodePool.node_graph.test_add,
        "add2",
        x=ng.ctx.y,
    ...
    ng.group_outputs.sum2 = ng.nodes.add2.outputs.result
...
def test_generate_group_inputs_outputs() -> None:
    """Test the group inputs and outputs of a node graph."""
    ng = NodeGraph(name="test_group_inputs_outputs")
    ...
    ng.generate_group_inputs()
    ng.generate_group_outputs()
    assert len(ng.group_inputs) == 2
    assert len(ng.group_outputs) == 2
    assert len(ng.links) == 6
    assert "add1.x" in ng.group_inputs
    assert "add2.result" in ng.group_outputs
