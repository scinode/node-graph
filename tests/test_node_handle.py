...
def test_nodehandle_inputs_outputs_view():
    spec = NodeSpec(
        ...
        executor=NodeExecutor(mode="module", module_path="math", callable_name="hypot"),
    )
    h = NodeHandle(spec)
    # views exist
    iv = h.inputs
...
def test_nodehandle_call_flow():
    ...
    spec = NodeSpec(
        ...
        inputs=ns(x=int, y=int),
        outputs=ns(sum=int),
        executor=NodeExecutor.from_callable(add),
    )
    h = NodeHandle(spec)

    # call with args & kwargs; our stubbed prepare_function_inputs will pack them
