...
def test_namespace_build_and_roundtrip():
    tm = ss.type_mapping  # module mapping is the ground truth
    ns = ss.namespace(
        a=int,
    ...
    assert ns2.fields["c"].fields["d"].identifier == ss.type_mapping[str]
...
def test_from_namespace_snapshot(monkeypatch):
    ...
    We stub the runtime sockets to avoid pulling the whole engine in.
    """
    ...
    live = FakeNS(
        "inputs",
        sockets={
            "x": FakeLeaf("x", ident=ss.type_mapping[int]),
            "inner": FakeNS(
                "inner", sockets={"y": FakeLeaf("y", ident=ss.type_mapping[str])}
            ),
        },
    )
    ...
    assert spec.identifier == ss.type_mapping["namespace"]
    assert set(spec.fields.keys()) == {"x", "inner"}
    assert spec.fields["x"].identifier == ss.type_mapping[int]
    assert spec.fields["inner"].identifier == ss.type_mapping["namespace"]
    assert spec.fields["inner"].fields["y"].identifier == ss.type_mapping[str]
